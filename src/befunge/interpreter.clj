...
(defn interpretar-comando [entorno comando]
  (if (:modo-cadena entorno)
    ...
    (case comando
      ...
      \_ (let [[a entorno] (desapilar entorno)]
           (assoc entorno :direccion (if (zero? a) :derecha :izquierda)))

      \| (let [[a entorno] (desapilar entorno)]
           (assoc entorno :direccion (if (zero? a) :abajo :arriba)))

      \! (let [[a entorno] (desapilar entorno)]
           (apilar entorno (if (zero? a) 1 0)))

      \` (let [[a entorno] (desapilar entorno)
               [b entorno] (desapilar entorno)]
           (apilar entorno (if (> b a) 1 0)))

      \" (update entorno :modo-cadena not)

      \. (let [[num entorno] (desapilar entorno)]
           (when num (print (str num " ")))
           entorno)

      \, (let [[chr entorno] (desapilar entorno)]
      ...
      \: (let [tope (or (first (:pila entorno)) 0)]
           (apilar entorno tope))

      \\ (let [[a entorno] (desapilar entorno)
               [b entorno] (desapilar entorno)]
           (-> entorno (apilar a) (apilar b)))
      ...
      \/ (let [[a entorno] (desapilar entorno)
               [b entorno] (desapilar entorno)]
           (apilar entorno (if (= a 0) 0 (quot b a))))

      \% (let [[a entorno] (desapilar entorno)
               [b entorno] (desapilar entorno)]
           (apilar entorno (if (= a 0) 0 (mod b a))))

      \# (mover entorno)

      \p (let [[a entorno] (desapilar entorno)
               [b entorno] (desapilar entorno)
               [c entorno] (desapilar entorno)]
           (update entorno :programa update-in [a b] (constantly (char c))))

      \g (let [[a entorno] (desapilar entorno)
               [b entorno] (desapilar entorno)]
      ...
      (if (Character/isDigit comando)
        (apilar entorno (Character/digit comando 10))
        entorno))))

(defn ejecutar-programa [programa]
  (loop [entorno (crear-entorno programa)]
    (if (= (get-in (:programa entorno) (:posicion entorno)) \@)
      nil
      (recur (-> entorno
                 (interpretar-comando (get-in (:programa entorno) (:posicion entorno)))
                 mover)))))